...
var janus = null;
var selfdefine = null;
var opaqueId = "selfdefine-"+Janus.randomString(12);
			...
			janus = new Janus(
				{
					...
					success: function() {
						...
						janus.attach(
							{
								plugin: "janus.plugin.selfdefine",
								opaqueId: opaqueId,
								success: function(pluginHandle) {
									$('#details').remove();
									selfdefine = pluginHandle;
									Janus.log("Plugin attached! (" + selfdefine.getPlugin() + ", id=" + selfdefine.getId() + ")");
									// Negotiate WebRTC
									var body = { "audio": true, "video": true };
									Janus.debug("Sending message (" + JSON.stringify(body) + ")");
									selfdefine.send({"message": body});
									Janus.debug("Trying a createOffer too (audio/video sendrecv)");
									selfdefine.createOffer(
										{
											...
											success: function(jsep) {
												Janus.debug("Got SDP!");
												Janus.debug(jsep);
												selfdefine.send({"message": body, "jsep": jsep});
											},
								...
								onmessage: function(msg, jsep) {
									...
									if(jsep !== undefined && jsep !== null) {
										...
										Janus.debug(jsep);
										selfdefine.handleRemoteJsep({jsep: jsep});
									}
								...
								onlocalstream: function(stream) {
									...
									if(selfdefine.webrtcStuff.pc.iceConnectionState !== "completed" &&
											selfdefine.webrtcStuff.pc.iceConnectionState !== "connected") {
								...
								onremotestream: function(stream) {
									...
									$('#toggleaudio').click(
										function() {
											...
											selfdefine.send({"message": { "audio": audioenabled }});
										});
									$('#togglevideo').click(
										function() {
											...
											selfdefine.send({"message": { "video": videoenabled }});
										});
									...
									$('#bitrate a').click(function() {
										...
										selfdefine.send({"message": { "bitrate": bitrate }});
										return false;
									});
									if(Janus.webRTCAdapter.browserDetails.browser === "chrome" || Janus.webRTCAdapter.browserDetails.browser === "firefox" ||
										...
										bitrateTimer = setInterval(function() {
											...
											var bitrate = selfdefine.getBitrate();
											//~ Janus.debug("Current bitrate is " + selfdefine.getBitrate());
...
function addSimulcastButtons(temporal) {
	...
	$('#sl-0').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			...
			selfdefine.send({message: { substream: 0 }});
		});
	$('#sl-1').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			...
			selfdefine.send({message: { substream: 1 }});
		});
	$('#sl-2').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			...
			if(!$('#sl-0').hasClass('btn-success'))
				$('#sl-0').removeClass('btn-primary btn-info').addClass('btn-primary');
			selfdefine.send({message: { substream: 2 }});
		});
	...
	$('#tl-0').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			...
			selfdefine.send({message: { temporal: 0 }});
		});
	$('#tl-1').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			...
			selfdefine.send({message: { temporal: 1 }});
		});
	$('#tl-2').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			...
			if(!$('#tl-0').hasClass('btn-success'))
				$('#tl-0').removeClass('btn-primary btn-info').addClass('btn-primary');
			selfdefine.send({message: { temporal: 2 }});
		});
}
